...
#    connection_string = "DRIVER={SQL Server};SERVER=" + configs['AERIESSQLServerSandbox'] + ";DATABASE=" + configs['AERIESDatabaseSB'] + ";UID=" + configs['AERIESUsername'] + ";PWD=" + configs['AERIESPassword'] + ";"
    ...
    print(TermDF)
    for index,row in TermDF.iterrows():
       bid = row["courseid"]
       current_course = canvas.get_course(bid)
       current_sections = current_course.get_sections()
       for section in current_sections:
            try:
               deletecross = section.decross_list_section()
               print('Decross listing a class ID ->' + str(bid))
               print(section)
            except CanvasException as e:
               if str(e) == "BadRequest":
                    print("Section not Cross-listed CourseID->" + str(bid))
    ...
    for i in TermDF.index:
        try:
            CourseToRename = canvas.get_course(TermDF['courseid'][i])
            TheTerm = TermDF['sistermid'][i]
            tempcoursename = CourseToRename.name + " (" + TheTerm[-1] + ")"
            print("Renaming Course to " + tempcoursename)
            CourseToRename.update(course={'name': tempcoursename})
        except CanvasException as e:
            if str(e) == "Not Found":
