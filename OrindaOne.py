...
if __name__ == '__main__':
    ...
    logger = logging.getLogger('OrindaONE')
...
SELECT 
    ...
    CONCAT('20', STU.U10, ' ', 
        CASE STU.SC
            WHEN 1 THEN 'LAS LOMAS'
            WHEN 2 THEN 'ACALANES'
            WHEN 3 THEN 'MIRAMONTE'
            WHEN 4 THEN 'CAMPOLINDO'
            WHEN 6 THEN 'ACIS'
            ELSE 'Unknown SC'
        END
    ) AS 'Grade',
...
WHERE STU.DEL = 0 
  ...
  AND STU.SC = 3 
    ...
    google_sheet_id = "1KOGkG5ekYfi2ujKoG9N4efLsFs6OXGFygGcYRCmIDmw"
    ...
    stat1 = gam.CallGAMCommand(['gam',
                                ...
                                uploadfilestring,
                                'csvsheet',
                                "Miramonte Student Contact Info"])
    if stat1 != 0:  
    ...
    if WasThereAnError:
        ...
        """
    if WasThereAnError:
        msg['Subject'] = f"🔴 ERROR! {configs['SMTPStatusMessage']} - Orinda ONE Info to Google Sheets {datetime.datetime.now().strftime("%I:%M%p on %B %d, %Y")}"
    else:
        msg['Subject'] = f"🟢 {configs['SMTPStatusMessage']} - Orinda ONE Info to Google Sheets {datetime.datetime.now().strftime("%I:%M%p on %B %d, %Y")}"
