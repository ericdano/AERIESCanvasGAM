...
groupusers = pd.DataFrame(group.users,columns=['id','name','login_id'])
print(groupusers)
...
#    print(SEM)
sql_query.to_csv('AllAeries.csv')
groupusers.to_csv('AllCanvas.csv')

common = sql_query.merge(groupusers,left_on='SEM',right_on='login_id', how='inner')
print('Difference---------')
print(common)
tempp = groupusers
common3 = tempp.merge(sql_query,left_on='login_id',right_on='SEM',how='inner')
print('Reversed----')
print(common3)
sql_query[~(sql_query['SEM'].isin(groupusers['login_id']))]
print('New diff')
print(sql_query)
